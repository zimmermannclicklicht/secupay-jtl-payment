...
class agws_secupay_flex_rg extends BasePaymentMethod
{
    /**
    ...
    /**
     * @param $kBestellung
     * @param $agws_secupay_flex_rg_due_text
     * @param $agws_secupay_flex_recipient_legal
     * @param $agws_secupay_flex_accountowner
     * @param $agws_secupay_flex_accountnumber
     * @param $agws_secupay_flex_bankcode
     * @param $agws_secupay_flex_bankname
     * @param $agws_secupay_flex_iban
     * @param $agws_secupay_flex_bic
     * @param $agws_secupay_flex_purpose
     * @param $agws_secupay_flex_payment_link
     * @param $agws_secupay_flex_payment_qr_image_url
     */
            ...
            if ($this->verifyNotification($order, $agws_secupay_flex_paymentHash, $args))
            {
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()->executeQuery(
                        "UPDATE tbestellung SET cAbgeholt='Y' WHERE kBestellung='".$order->kBestellung."'", 4)
            ...
            if ($this->verifyNotification($order, $agws_secupay_flex_paymentHash, $args, $agws_secupay_flex_antwort_json))
            {
                ...
                $this->secupayHelper->isShop4()
                    ? $oResTmp = Shop::DB()->executeQuery(
                        "SELECT cKommentar FROM tbestellung WHERE kBestellung='".$order->kBestellung."'", 8)
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()->executeQuery(
                        "UPDATE tbestellung SET cKommentar = '" . $agws_secupay_flex_Kommentar . "' WHERE kBestellung='".$order->kBestellung."'", 4)
                    : $GLOBALS['DB']->executeQuery(
                        "UPDATE tbestellung SET cKommentar = '" . $agws_secupay_flex_Kommentar . "' WHERE kBestellung='".$order->kBestellung."'", 4);
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()->executeQuery(
                        "UPDATE tbestellung SET cAbgeholt='N' WHERE kBestellung='".$order->kBestellung."'", 4)
                    : $GLOBALS["DB"]->executeQuery(
                        "UPDATE tbestellung SET cAbgeholt='N' WHERE kBestellung='".$order->kBestellung."'", 4);

                $this->secupayHelper->isShop4()
                    ? Shop::DB()->executeQuery(
                        "UPDATE xplugin_agws_secupay_flex_tsyslog SET cHash='".Shop::DB()->realEscape($_REQUEST['hash'])."',cTACode='".Shop::DB()->realEscape($agws_secupay_flex_antwort_json->data->trans_id)."',kAmountSecupay='".Shop::DB()->realEscape($agws_secupay_flex_antwort_json->data->amount)."',dPushDat=now() WHERE kBestellung='".$order->kBestellung."'", 4)
                    : $GLOBALS["DB"]->executeQuery(
                        "UPDATE xplugin_agws_secupay_flex_tsyslog SET cHash='".$GLOBALS["DB"]->realEscape($_REQUEST['hash'])."',cTACode='".$GLOBALS["DB"]->realEscape($agws_secupay_flex_antwort_json->data->trans_id)."',kAmountSecupay='".$GLOBALS["DB"]->realEscape($agws_secupay_flex_antwort_json->data->amount)."',dPushDat=now() WHERE kBestellung='".$order->kBestellung."'", 4);
                  ...
                  && ($x['scheme'] == $x1['scheme']) && ($x['host'] == $x1['host']) )
                {
                    ...
                    $this->secupayHelper->isShop4()
                        ? $oNotifyDate = Shop::DB()->executeQuery(
                            "SELECT dNotify FROM tzahlungsession WHERE kBestellung = " . intval($order->kBestellung), 8)
